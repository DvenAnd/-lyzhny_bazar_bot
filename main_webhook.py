...
- Веб-сервер под Render Free: принимает вебхуки от Telegram и публикует одобренные объявления в канал/чат.
- Функции: мастер подачи объявления (поля + фото/видео), предпросмотр, модерация админами, публикация.

Требования:
  python-telegram-bot[webhooks] >= 20,<21
  python-dotenv >= 1.0.0  (опционально; переменные окружения задаются в Render → Environment)

Переменные окружения (Render → Environment):
  BOT_TOKEN    : токен бота из @BotFather
  CHANNEL_ID   : @username канала ИЛИ числовой id чата/канала (-100xxxxxxxxxx)
  ADMIN_IDS    : числовые Telegram-ID админов, через запятую (пример: 111111111,222222222)
  MODE         : WEBHOOK
  WEBHOOK_URL  : https://<имя-сервиса>.onrender.com/telegram

Запуск локально (опционально для теста):
...
# ----- Conversation States -----
(CAT, ITEM, SIZE, CONDITION, PRICE, CITY, DELIVERY, CONTACT, DESCRIPTION, MEDIA, CONFIRM) = range(11)
...
async def set_contact(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Короткое описание (1–2 предложения: сезон, уровень, нюансы):")
    return DESCRIPTION
            ...
            if len(media_list) == 1:
                ...
                if kind == "photo":
                    await context.bot.send_photo(
                        chat_id=aid, photo=fid, caption=text, parse_mode=ParseMode.HTML, reply_markup=keyboard
                    )
                else:
                    await context.bot.send_video(
                        chat_id=aid, video=fid, caption=text, parse_mode=ParseMode.HTML, reply_markup=keyboard
                    )
            else:
                album = []
                for i, (kind, fid) in enumerate(media_list):
                    ...
                    if kind == "photo":
                        ...
                        album.append(InputMediaVideo(media=fid, caption=caption, parse_mode=ParseMode.HTML if caption else None))
                await context.bot.send_media_group(chat_id=aid, media=album)
